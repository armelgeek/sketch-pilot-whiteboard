...
class Config:
    ...
    # ── contour-wipe 上色模式专用 ──
    color_fill: str = "brush"         # 上色风格: "brush" 沿轨迹刷(默认，同一条笔迹路径，结构上不会画到空白处) | "contour-wipe" 轮廓感知自上而下扫描
    brush_speed_mult: float = 1.0     # brush 模式：色彩沿同一条笔迹路径追赶的速度倍数（1.0 = 与画面预算帧数同速，不加速）
    ...
    # ── 矢量化拟合（niveau 5, étape 1）──
    # vector_fit: True = ajuste une vraie courbe B-spline (scipy) au lieu du
    # lissage Chaikin (qui ne fait qu'arrondir les angles d'une polyligne
    # pixel). Repli silencieux sur Chaikin si scipy absent ou trait trop court.
    vector_fit: bool = True
    vector_smooth: float = 0.5          # facteur de lissage de la spline (plus haut = plus lisse/éloigné du bruit pixel)
    # ── style de stylo (façon Golpo "Pen/Stylus/Marker") ──
    # multiplicateur d'épaisseur de trait : 1.0 = tel quel (stylus, défaut),
    # <1 amincit (pen fin), >1 épaissit (marker). Voir _adjust_line_weight.
    pen_line_weight: float = 1.0
    # ── 笔尖朝向跟随笔迹切线 ──
    # hand_angle_offset: l'angle de pointage NEUTRE (degrés) de
    # drawing-hand.png -- mesuré par PCA sur les pixels du feutre (axe
    # principal + sens donné par la teinte brune de la pointe) : -138°
    # pour cet asset (pointe en haut-gauche, poignet en bas-droite). À
    # recalculer si l'asset change.
    # hand_angle_clamp: amplitude de rotation autorisée autour de cette
    # pose neutre (degrés), pour éviter un poignet qui se retourne de
    # façon anatomiquement improbable sur les traits qui repartent en
    # arrière.
    hand_angle_offset: float = -138.0
    hand_angle_clamp: float = 35.0
    hand_rotate: bool = False  # True = active la rotation selon la tangente du trait
...
class TipOverlay:
    ...
    def _rotated(self, angle_deg: float) -> tuple[np.ndarray, np.ndarray]:
        """
        Fait pivoter hand/mask autour du point d'ancrage (tip_px, tip_py),
        avec un cache par degré arrondi (l'angle varie en continu le long
        d'un trait, mais un pas d'1° est visuellement indiscernable et
        évite de refaire le warpAffine à chaque frame).
        """
        key = int(round(angle_deg)) % 360
        cached = self._rot_cache.get(key)
        if cached is not None:
            return cached
        m = cv2.getRotationMatrix2D((self.tip_px, self.tip_py), angle_deg, 1.0)
        hand_r = cv2.warpAffine(
            self.hand, m, (self.w, self.h),
            flags=cv2.INTER_LINEAR, borderMode=cv2.BORDER_CONSTANT, borderValue=(0, 0, 0),
        )
        mask_r = cv2.warpAffine(
            self.mask, m, (self.w, self.h),
            flags=cv2.INTER_LINEAR, borderMode=cv2.BORDER_CONSTANT, borderValue=0.0,
        )
        self._rot_cache[key] = (hand_r, mask_r)
        return hand_r, mask_r

    def stamp(self, canvas: np.ndarray, x: int, y: int, alpha: float = 1.0,
              angle_deg: float = 0.0) -> np.ndarray:
        """让素材的笔尖锚点对齐到画布坐标 (x, y)（即落墨点），支持 alpha 透明度。

        angle_deg: rotation de la main autour du point d'ancrage, en degrés,
        0 = orientation neutre de l'asset d'origine. À calibrer contre le
        vrai drawing-hand.png (offset + clamp) car l'angle "neutre" dépend
        de la pose dessinée dans l'asset -- pas déductible du code seul."""
        if alpha <= 0.0:
            return canvas
        hand, mask = (self._rotated(angle_deg) if abs(angle_deg) > 0.5
                      else (self.hand, self.mask))
        # 素材左上角 = 落墨点 - 笔尖偏移
        ...
        mask_region = mask[sy0:sy1, sx0:sx1] * alpha
        inv_region = 1.0 - mask_region
...
def _fit_bspline_stroke(
    points: list[tuple[float, float]], spacing: float, smooth_factor: float = 0.5
) -> list[tuple[float, float]]:
    """
    Ajuste une vraie courbe B-spline lissée à travers les points d'un trait,
    puis ré-échantillonne uniformément le long de cette courbe.

    Différence avec _chaikin_smooth : Chaikin ne fait que couper les coins
    d'une polyligne existante -- ça reste une approximation pixel qui
    "suit" le bruit du squelette. Une spline est une courbe mathématiquement
    continue ajustée aux points (avec un facteur de lissage qui l'autorise à
    s'écarter légèrement du bruit), évaluable à n'importe quelle résolution
    -- c'est la vraie étape "du pixel au vecteur" du tracé, indépendante de
    la taille de sortie.

    Repli silencieux sur les points d'entrée si le fit échoue (trop peu de
    points, points dégénérés, scipy absent) : le pipeline reste robuste
    même sans cette amélioration.
    """
    if len(points) < 4:
        return points
    try:
    ...
    # dédoublonne les points consécutifs identiques (cassent splprep)
    diffs = np.diff(pts, axis=0)
    keep = np.concatenate([[True], np.any(np.abs(diffs) > 1e-6, axis=1)])
    pts = pts[keep]
    if len(pts) < 4:
        return points
    ...
    try:
        ...
        length = _stroke_cumulative_length([tuple(p) for p in pts])[-1]
        n_samples = max(2, int(length / max(spacing, 0.5)))
