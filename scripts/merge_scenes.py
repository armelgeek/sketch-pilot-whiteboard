...
from transitions import VALID_TRANSITIONS, merge_with_transitions
...
def main(argv=None) -> int:
    ...
    p.add_argument(
        ...
        help="逗号分隔的转场列表，长度 = len(inputs)-1。"
             f"可选: {', '.join(sorted(VALID_TRANSITIONS))}。"
             "缺省或全部为 'cut' 时走原有硬切拼接。",
    )
    p.add_argument("--transition-ms", type=int, default=500, help="转场时长，默认 500ms")
    ...
    transitions = args.transitions.split(",") if args.transitions else []
    wants_transitions = transitions and any(t != "cut" for t in transitions)

    if wants_transitions and len(inputs) >= 2:
        if len(transitions) != len(inputs) - 1:
            print(
                f"[err] --transitions attend {len(inputs) - 1} valeurs, recu {len(transitions)}",
                file=sys.stderr,
            )
            return 1
        try:
            if merge_with_transitions(inputs, output, transitions, args.transition_ms):
                print(f"OUTPUT={output.resolve()}")
                return 0
            print("  [warn] Transitions indisponibles (ffmpeg/ffprobe manquant), repli sur cut sec.")
